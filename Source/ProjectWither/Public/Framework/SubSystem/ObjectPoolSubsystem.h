...
using FOrderNode =
TDoubleLinkedList<TObjectPtr<AActor>>::TDoubleLinkedListNode;
...
struct FObjectPool
{
    ...
    FObjectPool()
        : ActiveOrderList(
            MakeShared<TDoubleLinkedList<TObjectPtr<AActor>>>()),
        ActiveNodeMap(
            MakeShared<TMap<TObjectPtr<AActor>, FOrderNode*>>())
    {
    }
    ...
    TSharedPtr<TDoubleLinkedList<TObjectPtr<AActor>>>
        ActiveOrderList;

    // Actor별 활성 순서 노드
    TSharedPtr<TMap<TObjectPtr<AActor>, FOrderNode*>>
        ActiveNodeMap;
...
UCLASS()
class PROJECTWITHER_API UObjectPoolSubsystem
...
public:
    ...
    bool RegisterPoolDataAsset(
        const UObjectPoolDataAsset* InDataAsset,
        bool bWarmup = false);

    UFUNCTION(BlueprintCallable)
    bool UnregisterPoolDataAsset(
        const UObjectPoolDataAsset* InDataAsset);
    ...
    UFUNCTION(BlueprintCallable)
    AActor* Spawn(
        TSubclassOf<AActor> InClassType,
        const FTransform& InTransform);

    template<typename T>
    T* Spawn(
        TSubclassOf<T> InClassType,
        const FTransform& InTransform)
    {
        return Cast<T>(
            Spawn(
    ...
    {
        return Cast<T>(
            Spawn(T::StaticClass(), InTransform));
    }
