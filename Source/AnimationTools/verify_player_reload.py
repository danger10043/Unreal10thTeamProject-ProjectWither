...
a=unreal.load_asset(base+'AS_GunReload_Player')
...
assert samples[2]['right_hand'][2]<110,'Right hand did not reach magazine pickup position'
report={'duration':a.get_play_length(),'bones_checked':len(names),'frames_checked':145,'max_endpoint_position_error_cm':max_pos,'min_endpoint_quaternion_dot':min_qdot,'max_hand_frame_step_cm':max_step,'max_bone_frame_angle_degrees':max_angle,'max_angle_bone':max_angle_bone,'samples':samples}
with open(os.path.join(os.path.dirname(__file__),'player_reload_verified.json'),'w') as f: json.dump(report,f,indent=2)
